...
def test_list():
    cache = RedisCache(host="localhost", port=6380, db=0, expire_mins=60*24, clear_cache=False)
    #cache = MemoryCache()
    dartapi = DartAPI(keys=[""], cache=cache)

    r = dartapi.get_disclosure_list(end_de="20200103", max_page=10, use_cache=True)
